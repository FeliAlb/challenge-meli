...
Paso 1: 
 - Levantar MITRE MCP (Terminal 1): **mitre-mcp --http --host localhost --port 8000**
...
La solución puede ejecutarse dentro de un contenedor Docker.
Es importante tener en cuenta que el contenedor ejecuta únicamente el pipeline de detección (Analyzer → Classifier → Reporter).
El servidor MITRE ATT&CK MCP debe ejecutarse externamente y estar disponible antes de iniciar el contenedor.

- Iniciar el servidor MITRE MCP (requerido). En una terminal separada, ejecutar: **mitre-mcp --http --host localhost --port 8000**
- En Windows: **C:\Users\<usuario>\AppData\Local\Programs\Python\Python312\Scripts\mitre-mcp.exe --http --host localhost --port 8000**
- El endpoint utilizado por la aplicación es: **http://localhost:8000/mcp**

Esta terminal debe permanecer activa durante la ejecución del contenedor.

- Construir la imagen Docker. Desde la raíz del proyecto: **docker build -t meli-datasec .**
- Ejecutar el contenedor: **docker run meli-datasec**

Consideraciones Importantes:
- El contenedor asume que el servidor MITRE MCP ya se encuentra en ejecución.
- Si el servidor MCP no está disponible, el pipeline continuará funcionando, pero el enriquecimiento de MITRE ATT&CK puede no completarse.
- Cada ejecución generará una carpeta **runs/<session_id>/** con los resultados correspondientes.
